...
def metrics(model, evaluate_set:dict):
    ...
    for user_id, item_dic in enumerate(evaluate_set):
        ...
        rank = np.concatenate([prediction,temp],axis=1).tolist().sort(reverse=True)
        DCG = 0
        for i,(_,score) in rank:
            DCG += (2**score - 1) / math.log2(i+2)  # 因为 i 从 0 开始所以额外加1
        iDCG = 0
        iRank = list(item_dic.values()).sort(reverse=True)
        for i, score in iRank:
